...
function deleteError() {
    var n = document.getElementById('answers').value;
    if (!n) {
...
function setTimer(time) {
    ...
    {
       hideDiv('timer_div');
       document.getElementById('statistics_div').innerHTML = "Please submit your answer!";
...
function setTimer2() {
   ...
   if (isset($_GET['submit'])) { ?>
    document.getElementById('statistics_div').innerHTML = "Please wait...";
    <?php } ?>
    if (seconds_left2 < -1) {
    	hideDiv('button_sub');
        ...
        $result = mysqli_query($conn, $sql);
    	?>
       document.getElementById('statistics_div').innerHTML = "<div style='color: #A9CCE3;'><u>Statistics:</u><br/>Total answers: <?= $row['Total'] ?><br/>Success Rate: <?= $success_rate ?>%</div>";
       clearInterval(interval);
    }
}, 1000);
...
<table class="tableone">
    ...
    <tr><td colspan="2">
        ...
        <form method="post" action="php_answer.php">
            <select id="answers" name="answers" class="selection" multiple>
...
<h2><center><div id="statistics_div"></div></center></h2>
